...
#!/bin/bash
# frpc 增強功能模組 - 從 kejilion.sh 4.4.10-cong86.1 提取
# 包含: configure_frpc, 輔助校驗函數, add_forwarding_service,
...
add_forwarding_service() {
	...
	while true; do
		...
		if [ -f /home/frp/frpc.toml ] && grep -qE "^\[$service_name\]$" /home/frp/frpc.toml; then
			echo "新增失敗：服務名稱$service_name已存在，請重新輸入其他名稱。"
			continue
		fi
		break
	done
	...
	while true; do
		...
		case "$service_type" in
			...
			*) echo "新增失敗：轉發類型僅支援 tcp/udp/http/https，請重新輸入。" ;;
		esac
	done
	...
	while true; do
		...
		if ! is_valid_port "$local_port"; then
			echo "新增失敗：內網連接埠必須是 1-65535 的數字，請重新輸入。"
			continue
		fi
		break
	done
		...
		http|https)
			while true; do
				...
				if ! is_valid_domain "$custom_domains"; then
					echo "新增失敗：域名格式不正確，請輸入純域名，不要帶 http://、https://、連接埠或路徑。"
					continue
				fi
				duplicate_domain=0
...
dd_xitong() {
		...
		dd_xitong_3() {
		  echo -e "重裝後初始使用者名稱:${gl_huang}root${gl_bai}  初始密码: ${gl_huang}123@@@${gl_bai}初始連接埠:${gl_huang}22${gl_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
...
kpanel_node_activate() {
	...
	if [ -f "$KPANEL_NODE_TERMINAL_CONFIG" ]; then
		if ! "$KPANEL_NODE_SYSTEMCTL" start kejilion-node-terminal.service; then
			echo "KPanel 輕量節點終端 broker 啟動失敗；檔案管理和遙測服務仍將持續。" >&2
		fi
	fi
	if ! "$KPANEL_NODE_SYSTEMCTL" start kejilion-node-ssh-login.service; then
		echo "KPanel SSH 登入擷取服務啟動失敗；普通遙測仍將持續。" >&2
	fi
	...
	if [ -f "$KPANEL_NODE_TERMINAL_CONFIG" ] && ! "$KPANEL_NODE_SYSTEMCTL" is-active kejilion-node-terminal.service >/dev/null; then
		echo "KPanel 輕量節點終端 broker 目前無法使用；檔案管理和遙測服務仍在運作。" >&2
	fi
	...
	if ! "$KPANEL_NODE_SYSTEMCTL" is-active kejilion-node-ssh-login.service >/dev/null; then
		echo "KPanel SSH 登入擷取服務目前無法使用；普通遙測仍在運作。" >&2
	fi
...
kpanel_node_join() {
	...
	if ! kpanel_node_write_updater || ! "$KPANEL_NODE_UPDATER" "$update_mode"; then
		...
		echo "KPanel 輕量節點安裝未完成；請根據上面的提示處理後，再次執行接入命令。" >&2
		return 1
	fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}89.  ${color89}FileCodeBox文件快递                 ${gl_kjlan}90.  ${color90}matrix去中心化聊天協議"
	  echo -e "${gl_kjlan}-------------------------"
	  ...
	  echo -e "${gl_kjlan}97.  ${color97}WireGuard组网(服务端)               ${gl_kjlan}98.  ${color98}WireGuard組網(客戶端)"
	  ...
	  echo -e "${gl_kjlan}117. ${color117}99CDN自建CDN管理平台${gl_kjlan}118. ${color118}99DNS智慧調度服務"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}第三方應用程式列表"
  	  echo -e "${gl_kjlan}想要让你的应用出现在这里？查看開發者指南:${gl_huang}https://dev.kejilion.sh/${gl_bai}"
	...
	while true; do
		...
		case $choice in
			...
			2)
				...
				if ip link show "$nic" &>/dev/null; then
					ip link set "$nic" down && echo "✔ 網路卡$nic 已禁用"
				else
					echo "✘ 網路卡不存在"
				fi
				read -erp "按回车继续..."
...
kpanel_system_resource_run_locked() (
	...
	lock_file="$(kpanel_system_resource_prepare_lock_file)" || {
		kpanel_system_resource_error "system-resource 鎖路徑不安全或無法建立"
		kpanel_system_resource_emit failed "$(kpanel_system_resource_best_version "$resource" "$name")"
		return 1
	}
	exec 9<>"$lock_file" || {
...
kpanel_disk_management_require_platform() {
	...
	if [ "${KJ_DISK_MANAGEMENT_NONINTERACTIVE:-}" != "1" ]; then
		KPANEL_DISK_MANAGEMENT_REQUIRE_MESSAGE="KPanel disk-management 協定環境未啟用"
		return 2
	fi
	if [ "$EUID" -ne 0 ]; then
		KPANEL_DISK_MANAGEMENT_REQUIRE_MESSAGE="KPanel disk-management 協定必須以 root 執行"
		return 2
	fi
...
kpanel_disk_management_prepare_device() {
	...
	rc=$?
	if [ "$rc" -ne 0 ]; then
		KPANEL_DISK_MANAGEMENT_PREPARE_MESSAGE="無法按 MAJ:MIN 唯一解析安全性區塊設備"
		return 1
	fi
	kpanel_disk_management_validate_device_common
	rc=$?
	case "$rc" in
		...
		3)
			...
			;;
		*)
			KPANEL_DISK_MANAGEMENT_PREPARE_MESSAGE="無法可靠讀取設備屬性"
			return 1
			;;
	esac
}
...
kpanel_disk_management_mount_action() {
	...
	case "$KPANEL_DISK_MANAGEMENT_FSTYPE" in
		swap|LVM2_member|linux_raid_member|crypto_LUKS|zfs_member)
			kpanel_disk_management_reply needs-attention "$device_id" "設備簽章不是可直接掛載的檔案系統" "" 3
			return $?
			;;
	esac
	...
	if [ "$rc" -eq 0 ]; then
		[ "$target" = "$device_id" ] || {
			kpanel_disk_management_reply needs-attention "$device_id" "目標已被其他裝置佔用" "" 3
			return $?
		}
		if [ "$persist" = 1 ]; then
			...
			if [ "$source_rc" -ne 0 ]; then
				if [ "$source_rc" -eq 2 ]; then
					kpanel_disk_management_reply needs-attention "$device_id" "設備缺少 UUID/PARTUUID，無法安全持久化" "" 3
				else
					kpanel_disk_management_reply failed "$device_id" "無法讀取設備持久化標識" "" 1
				fi
				return $?
			fi
			...
			if [ "$fstab_rc" -ne 0 ]; then
				if [ "$KPANEL_DISK_MANAGEMENT_FSTAB_ROLLBACK_FAILED" = true ]; then
					kpanel_disk_management_reply rollback-failed "$device_id" "fstab 更新失敗且設定回滾失敗，需手動恢復" "$backup" 4
				elif [ "$fstab_rc" -eq 2 ]; then
					kpanel_disk_management_reply needs-attention "$device_id" "fstab 存在裝置或掛載點衝突" "$backup" 3
				else
					kpanel_disk_management_reply failed "$device_id" "fstab 持久化失敗，原即時掛載保持不變" "$backup" 1
				fi
				return $?
			fi
			if [ "$KPANEL_DISK_MANAGEMENT_FSTAB_CHANGED" = true ]; then
				kpanel_disk_management_reply applied "$device_id" "即時掛載未變化，持久化配置已套用" "$backup" 0
			else
				kpanel_disk_management_reply unchanged "$device_id" "設備已按目標掛載並持久化" "" 0
			fi
			return $?
		fi
		kpanel_disk_management_reply unchanged "$device_id" "設備已依目標掛載" "" 0
		return $?
	elif [ "$rc" -ne 1 ]; then
		kpanel_disk_management_reply failed "$device_id" "無法可靠讀取目標掛載狀態" "" 1
		return $?
	fi
	if ! kpanel_disk_management_mountpoint_path_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" true; then
		kpanel_disk_management_reply needs-attention "$device_id" "未掛載目標不是 root 獨佔的安全目錄路徑" "" 3
		return $?
	fi
	kpanel_disk_management_device_is_mounted "$device_id"
	mounted_rc=$?
	if [ "$mounted_rc" -eq 0 ]; then
		kpanel_disk_management_reply needs-attention "$device_id" "設備已掛載在其他目標，拒絕重複掛載" "" 3
		return $?
	elif [ "$mounted_rc" -ne 1 ]; then
		kpanel_disk_management_reply failed "$device_id" "無法可靠讀取設備掛載狀態" "" 1
		return $?
	fi
	if [ -e "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" ] || [ -L "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" ]; then
		[ -d "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" ] && [ ! -L "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" ] || {
			kpanel_disk_management_reply needs-attention "$device_id" "目標不是安全的真實目錄" "" 3
			return $?
		}
	else
		parent="$(dirname -- "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT")" || {
			kpanel_disk_management_reply failed "$device_id" "無法解析掛載點父目錄" "" 1
			return $?
		}
		[ -d "$parent" ] && [ ! -L "$parent" ] || {
			kpanel_disk_management_reply needs-attention "$device_id" "掛載點父目錄不存在或不安全" "" 3
			return $?
		}
		if ! (umask 077; mkdir -- "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT") >/dev/null 2>&1; then
			kpanel_disk_management_reply failed "$device_id" "建立掛載點失敗" "" 1
			return $?
		fi
		created=true
		if ! chown 0:0 "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" >/dev/null 2>&1 ||
			...
			kpanel_disk_management_reply failed "$device_id" "無法設定新掛載點安全性屬性" "" 1
			return $?
		fi
		if ! kpanel_disk_management_mountpoint_path_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
			kpanel_disk_management_cleanup_new_mountpoint "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"
			kpanel_disk_management_reply failed "$device_id" "新掛載點安全性屬性複核失敗" "" 1
			return $?
		fi
		if ! kpanel_disk_management_record_mountpoint "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"; then
			kpanel_disk_management_cleanup_new_mountpoint "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"
			kpanel_disk_management_reply failed "$device_id" "無法記錄 KPanel 建立的掛載點" "" 1
			return $?
		fi
	fi
	content="$(find "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" || {
		...
		kpanel_disk_management_reply failed "$device_id" "無法安全檢查掛載點內容" "" 1
		return $?
	}
	if [ -n "$content" ]; then
		[ "$created" = true ] && kpanel_disk_management_cleanup_new_mountpoint "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"
		kpanel_disk_management_reply needs-attention "$device_id" "掛載點非空，拒絕覆蓋現有內容" "" 3
		return $?
	fi
	if [ "$persist" = 1 ]; then
		kpanel_disk_management_persistence_source "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH"
	...
	if ! kpanel_disk_management_mountpoint_path_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
		[ "$created" = true ] && kpanel_disk_management_cleanup_new_mountpoint "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"
		kpanel_disk_management_reply needs-attention "$device_id" "掛載前安全複核失敗" "" 3
		return $?
	fi
	if ! mount --source "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH" --target "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" >&2; then
		if kpanel_disk_management_rollback_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$created" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"; then
			kpanel_disk_management_reply failed "$device_id" "掛載指令失敗，未保留部分狀態" "" 1
		else
			kpanel_disk_management_reply rollback-failed "$device_id" "掛載命令失敗且無法確認或清理部分掛載" "" 4
		fi
		return $?
	fi
	if ! kpanel_disk_management_mountpoint_parent_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
		if kpanel_disk_management_rollback_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$created" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"; then
			kpanel_disk_management_reply failed "$device_id" "掛載後安全性屬性複核失敗，已回滾" "" 1
		else
			kpanel_disk_management_reply rollback-failed "$device_id" "掛載後安全性屬性複核失敗且回滾失敗" "" 4
		fi
		return $?
	fi
	target="$(kpanel_disk_management_target_device "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT")"
	...
	if [ "$rc" -ne 0 ] || [ "$target" != "$device_id" ]; then
		if kpanel_disk_management_rollback_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$created" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX"; then
			kpanel_disk_management_reply failed "$device_id" "掛載完成態回讀失敗，已回滾" "" 1
		else
			kpanel_disk_management_reply rollback-failed "$device_id" "掛載完成態回讀失敗且回滾失敗" "" 4
		fi
		return $?
	fi
	if [ "$persist" = 1 ]; then
		...
		if [ "$fstab_rc" -ne 0 ]; then
			if ! kpanel_disk_management_rollback_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" "$created" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT_HEX" ||
				...
				kpanel_disk_management_reply rollback-failed "$device_id" "fstab 持久化失敗且回滾未完整完成" "$backup" 4
			elif [ "$fstab_rc" -eq 2 ]; then
				kpanel_disk_management_reply needs-attention "$device_id" "fstab 有衝突，即時掛載已回滾" "$backup" 3
			else
				kpanel_disk_management_reply failed "$device_id" "fstab 持久化失敗，即時掛載已回滾" "$backup" 1
			fi
			return $?
		fi
	fi
	backup="${KPANEL_DISK_MANAGEMENT_FSTAB_BACKUP:-}"
	kpanel_disk_management_reply applied "$device_id" "設備已掛載並完成狀態回讀" "$backup" 0
}
...
kpanel_disk_management_unmount_action() {
	...
	if ! kpanel_disk_management_decode_mountpoint "$mountpoint_hex"; then
		kpanel_disk_management_reply failed "$device_id" "掛載點編碼、規範路徑或保護路徑校驗失敗" "" 2
		return $?
	fi
	if ! kpanel_disk_management_mountpoint_parent_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
		kpanel_disk_management_reply needs-attention "$device_id" "卸載目標路徑包含符號連結、非 root 目錄或可被非 root 寫入" "" 3
		return $?
	fi
	kpanel_disk_management_prepare_device "$device_id"
	rc=$?
	...
	if [ "$rc" -eq 1 ]; then
		...
		mounted_rc=$?
		if [ "$mounted_rc" -eq 0 ]; then
			kpanel_disk_management_reply needs-attention "$device_id" "設備掛載在其他目標，拒絕卸載" "" 3
		elif [ "$mounted_rc" -eq 1 ]; then
			kpanel_disk_management_reply unchanged "$device_id" "設備未掛載在指定目標" "" 0
		else
			kpanel_disk_management_reply failed "$device_id" "無法可靠讀取設備掛載狀態" "" 1
		fi
		return $?
	...
	[ "$target" = "$device_id" ] || {
		kpanel_disk_management_reply needs-attention "$device_id" "指定目標由其他裝置佔用" "" 3
		return $?
	}
	if [ "$remove_persistence" = 1 ]; then
		...
		if [ "$source_rc" -ne 0 ]; then
			if [ "$source_rc" -eq 2 ]; then
				kpanel_disk_management_reply needs-attention "$device_id" "設備缺少 UUID/PARTUUID，無法精確移除持久化記錄" "" 3
			else
				kpanel_disk_management_reply failed "$device_id" "無法讀取設備持久化標識" "" 1
			fi
			return $?
	...
	if ! kpanel_disk_management_mountpoint_parent_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
		kpanel_disk_management_reply needs-attention "$device_id" "卸載前安全複核失敗" "" 3
		return $?
	fi
	if ! umount -- "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" >&2; then
		kpanel_disk_management_reply needs-attention "$device_id" "普通卸載失敗，設備可能正被使用" "" 3
		return $?
	fi
	target="$(kpanel_disk_management_target_device "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT")"
	rc=$?
	if [ "$rc" -ne 1 ]; then
		kpanel_disk_management_reply failed "$device_id" "卸載完成態回讀失敗" "" 1
		return $?
	fi
	if ! kpanel_disk_management_mountpoint_path_secure "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" false; then
		if kpanel_disk_management_restore_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT"; then
			...
			kpanel_disk_management_reply rollback-failed "$device_id" "卸載後安全性屬性複核失敗且即時掛載恢復失敗" "" 4
		fi
		return $?
	fi
	if [ "$remove_persistence" = 1 ]; then
		...
		if [ "$fstab_rc" -ne 0 ]; then
			if ! kpanel_disk_management_restore_live_mount "$device_id" "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH" "$KPANEL_DISK_MANAGEMENT_MOUNTPOINT" ||
				[ "$KPANEL_DISK_MANAGEMENT_FSTAB_ROLLBACK_FAILED" = true ]; then
				kpanel_disk_management_reply rollback-failed "$device_id" "持久化移除失敗且即時掛載回滾失敗" "$backup" 4
			elif [ "$fstab_rc" -eq 2 ]; then
				kpanel_disk_management_reply needs-attention "$device_id" "fstab 有衝突，即時掛載已恢復" "$backup" 3
			else
				kpanel_disk_management_reply failed "$device_id" "持久化移除失敗，即時掛載已恢復" "$backup" 1
			fi
			return $?
		fi
	fi
	...
	backup="${KPANEL_DISK_MANAGEMENT_FSTAB_BACKUP:-}"
	kpanel_disk_management_reply applied "$device_id" "設備已從指定目標卸載" "$backup" 0
}
...
kpanel_disk_management_format_action() {
	...
	if [ -z "$command_name" ] || ! kpanel_disk_management_command_available "$command_name"; then
		kpanel_disk_management_reply needs-attention "$device_id" "缺少所選檔案系統的格式化工具" "" 3
		return $?
	fi
	case "$requested_fstype" in
	...
	rc=$?
	if [ "$rc" -ne 0 ]; then
		kpanel_disk_management_reply failed "$device_id" "格式化指令失敗，未宣告可回滾" "" 1
		return $?
	fi
	...
	actual="$(kpanel_disk_management_probe_fstype "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH")" || {
		kpanel_disk_management_reply failed "$device_id" "格式化後無法重讀檔案系統類型" "" 1
		return $?
	}
	[ "$actual" = "$requested_fstype" ] || {
		kpanel_disk_management_reply failed "$device_id" "格式化後檔案系統類型回讀不一致" "" 1
		return $?
	}
	kpanel_disk_management_reply applied "$device_id" "格式化已完成並透過檔案系統類型回讀" "" 0
}
...
kpanel_disk_management_check_action() {
	...
	fstype="$(kpanel_disk_management_probe_fstype "$KPANEL_DISK_MANAGEMENT_DEVICE_PATH")" || {
		kpanel_disk_management_reply needs-attention "$device_id" "無法辨識可檢查的檔案系統" "" 3
		return $?
	}
	case "$fstype" in
...
kpanel_disk_management_dispatch() {
	...
	case "$action" in
		mount|unmount)
			[ "$#" -eq 3 ] && [[ "$1" =~ ^[0-9]+:[0-9]+$ ]] &&
				...
				[[ "$3" =~ ^[01]$ ]] || {
				kpanel_disk_management_reply failed "$device_id" "mount/unmount 參數無效" "" 2
				return $?
			}
			;;
		format)
	...
	lock_file="$(kpanel_disk_management_prepare_lock_file)" || {
		kpanel_disk_management_reply failed "$device_id" "disk-management 鎖定路徑不安全或無法建立" "" 1
		return $?
	}
	exec 9<>"$lock_file" || {
	...
	if ! flock -w 5 -x 9 >/dev/null 2>&1; then
		exec 9>&-
		kpanel_disk_management_reply conflict "$device_id" "disk-management 寫鎖等待超時" "" 2
		return $?
	fi
...
kpanel_account_dispatch() {
	...
	case "$action" in
		...
		create|set-password|add-key|delete-key|set-role|set-ssh-policy|disable-root|create-admin-disable-root|delete)
			...
			;;
		*) kpanel_account_error "不支援的 account-management 動作"; kpanel_account_emit failed; return 2 ;;
	esac
}
		...
		case "$Limiting" in
		   ...
		   24) # 复制文件目录
			 ...
			 25) # 传送文件至远端服务器
				...
				kj_ssh_read_host_user_port "请输入远端服务器IP: " "请输入远端服务器用户名 (默认root): " "請輸入登入連接埠 (預設22):" "root" "22"
				local remote_ip="$KJ_SSH_HOST"
